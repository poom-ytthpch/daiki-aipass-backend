...
func resolveResponseLanguage(body []byte) responseLanguagePreference {
	...
	for i := len(messages) - 1; i >= 0; i-- {
		...
		if text == "" || isAutomaticAttachmentPrompt(text) {
			continue
		}
	...
	for i := len(messages) - 1; i >= 0 && seen < 8; i-- {
		...
		if latestCode == "" {
			latestCode = detected.Code
		}
		votes[detected.Code]++
		names[detected.Code] = detected.Name
		seen++
	}
	bestCode, bestCount := "", 0
	for code, count := range votes {
		if count > bestCount || (count == bestCount && code == latestCode) {
			bestCode, bestCount = code, count
		}
	}
	if bestCode != "" {
		return responseLanguagePreference{Code: bestCode, Name: names[bestCode], Source: "conversation"}
	}
	return responseLanguagePreference{}
}
