...
func (a *app) enrichChatWithResearch(ctx context.Context, body []byte) ([]byte, researchMetadata, error) {
	...
	if len(sources) > 0 {
		...
		priceFocused := researchPriceOnlyIntent(query, prefs)
		if prefs.Depth == "deep" && !priceFocused {
		...
		if priceFocused {
			b.WriteString("\nPRICE-FOCUSED OUTPUT: The latest user message is asking for current/latest price. Override the normal deep-report format: return a compact list/table of the requested current price/model lineup, followed by at most one short conflict note only when useful. Do not add specifications, charging, warranty, insurance, accessories, financing, promotional benefits, or historical/pre-discount/list prices unless the user explicitly asked for them. Historical/list/pre-discount numbers may appear inside the evidence only as context; when a current price is established, DO NOT repeat those older numbers in the final answer. If current sources genuinely conflict, report the dated conflict without speculating about the cause and prefer the newest applicable primary/local evidence. Never label a conflicting value as a typo, printing error, rounding difference, stale data, or other cause unless that cause is explicitly stated in the supplied source text.\n")
		}
